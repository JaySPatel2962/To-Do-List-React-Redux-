...
function App() {
	...
	const viewPendingTask = useSelector(
		(state) => state.filters.value.viewPendingTask
	);
	const viewCompletedTask = useSelector(
		(state) => state.filters.value.viewCompletedTask
	);
			...
			view.push(
				<li key={index} className="todolist__task">
					<label
						...
						className={`${task.completed ? "label--textdecorator" : ""}`}
					>
						<input
							type="checkbox"
							id={`${task.timestamp}`}
							name={`${"task" + index}`}
							onChange={() => dispatch(completeTask(task.timestamp))}
					...
					<button
						...
						onClick={() => dispatch(deleteTask(task.timestamp))}
					>
						delete
					</button>
				</li>
		...
		<div className="App">
			<div id="todolist" className="todolist">
				...
				<div className="buttons buttons--width">
					<Button name="deleteAll" onClick={() => dispatch(deleteAll())} />
					<Button
						name={latestFirst ? "Latest First" : "Latest Last"}
				...
				<div id="buttons" className="buttons">
					<button
						className={`buttons__btn ${
							viewCompletedTask ? "buttons__btn--clicked" : ""
						}`}
					...
					<button
						className={`buttons__btn ${
							viewPendingTask ? "buttons__btn--clicked" : ""
						} `}
					...
					<button
						className={`buttons__btn  ${
							!viewCompletedTask && !viewPendingTask
